...
import com.example.android.navigation.databinding.Lesson6FragmentBinding
...
class Lesson6Fragment: Fragment() {
    ...
    lateinit var binding: Lesson6FragmentBinding
    override fun onCreateView(
            inflater: LayoutInflater, container: ViewGroup?,
            savedInstanceState: Bundle?
    ): View? {
        binding = DataBindingUtil.inflate(
                inflater, R.layout.lesson6_fragment, container, false)

        val font = Typeface.createFromAsset(context!!.assets, "majallab.ttf")

        val spannable = SpannableString("Toi marbutaning asosi 'ت' undoshi ko'rinishidagi 'te' ('ﺓ  ﺔ')  harfidir. Bu alomat muannas (ayol) jinsiga dalolat qiluvchi so'zlar oxirida qo'llaniladi.Toi marbuta bilan to'htalganda 'ha' tovushi bo'lib o'qiladi. Misollar:\n")
        spannable.setSpan(
                RelativeSizeSpan(2f),
                spannable.indexOf("ت"), // start
                spannable.indexOf("ت") + 1, // end
                Spannable.SPAN_EXCLUSIVE_INCLUSIVE
        )
        spannable.setSpan(
                RelativeSizeSpan(2f),
                spannable.indexOf("ﺔ"), // start
                spannable.indexOf("ﺔ") + 1, // end
                Spannable.SPAN_EXCLUSIVE_INCLUSIVE
        )
        spannable.setSpan(
                RelativeSizeSpan(2f),
                spannable.indexOf("ﺓ"), // start
                spannable.indexOf("ﺓ") + 1, // end
                Spannable.SPAN_EXCLUSIVE_INCLUSIVE
        )

        binding.textToiMartuba.text = spannable
        ...
        binding.text.text = context!!.resources.getString(R.string.toi_martuba)
        binding.text.typeface = font
        mPlayer_1 = MediaPlayer.create(context, R.raw.toi_martuba)
